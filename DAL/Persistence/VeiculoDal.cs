...
namespace DAL.Persistence
{
    public class VeiculoDal : Conexao
    {
        ...
        public bool BuscarPelaPlaca(string placa)
        {
            try
            {
                AbrirConexao();
                Cmd = new SqlCommand("select Placa from Tb_Veiculo where Placa = @Placa", Con);
            ...
            {
                throw new Exception("Erro: VeiculoDal: BuscarPelaPlaca(string placa) => " + e.Message);
            }
            finally
            {
                FecharConexao();
